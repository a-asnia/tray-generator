...
export function buildContainer(c, conn, opts = {}) {
  ...
  // роль (рельс/паз) теперь у каждого узла своя — стенка возвращается
  // ровно там, где стоит рельс; в female-узлах её заменяет паз
  for (const [sideKey, units] of [["s", uS], ["n", uN], ["e", uE], ["w", uW]])
    for (const u of units) if (u.male) zoneWall(sideKey, u);
